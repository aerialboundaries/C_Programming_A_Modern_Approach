...
/* Maintains a parts database (array version) pp-22-08*/
...
int main(void) {
  ...
  for (;;) {
    ...
    scanf(" %c", &code); // space before %c skips the previously left over space
 ...
 *        order in which they are eintered into the
 *        database.
 ************************************************************************/
void print(void) {
  int i;
...
void dump(void) {
  printf("Enter name of output file: "); // ex iventory.dat
  char filename[FILENAME_LEN + 1];       // as array
...
void restore(void) {
  printf("Enter name of input file: "); // ex iventory.dat
