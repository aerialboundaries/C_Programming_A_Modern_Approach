...
void push(Stack s, int i)
{
    if (is_full(s)) {
        ...
        int *new_contents = malloc(new_size * sizeof(int));
        if (new_contents == NULL)
            terminalte("Error in push: stack could not be expanded.");
 ...
 *   if (is_full(s)) {
 *     int new_size = s->size * 2;
 *     int *temp = realloc(s-contents, new_size * sizeof(int));
 *     if (temp == NULL)
 *       terminate("Error in push: stack could not be expanded.");
 *
 *     s->contents = temp;
 *     s->size = new_size;
 *   }
