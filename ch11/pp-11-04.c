 ...
 * 2026-02-23
 *
 * Modify the poder.c program of Section 10.5 by moving all external variables
 * into main and modifying functions so that they communicate by passing
 * arguments. The analyze_hand function needs to change the straight, flusch,
 * four, three, and pairs vriables, so it will have to be passed pointers
 * to those variables.
 */
...
void analyze_hand(int num_in_rank[NUM_RANKS], int num_in_suite[NUM_SUITS],
...
/*************************************************************************
 * read_dards: Reads the cards into the external variables num_in_rank   *
...
void read_cards(int num_in_rank[NUM_RANKS], int num_in_suit[NUM_SUITS]) {
  ...
  // Initialize num_in_rank array
  for (rank = 0; rank < NUM_RANKS; rank++) {
    num_in_rank[rank] = 0;
    // Initialize card_exists array
    for (suit = 0; suit < NUM_SUITS; suit++)
      card_exists[rank][suit] = false;
  }
  // initialize num_in_suit array
  for (suit = 0; suit < NUM_SUITS; suit++)
    num_in_suit[suit] = 0;
  ...
  while (cards_read < NUM_CARDS) {
    ...
    // if there is 3rd char, ignore it as bad card.
    while ((ch = getchar()) != '\n')
      if (ch != ' ')
        bad_card = true;

    // if not error, update num_in_rank, num_in_suit, card_exists
    // and incremente cards_read.
    if (bad_card)
...
/*************************************************************************
 * analyze_hand: Determines whether the hand contains a straint, a flush
 * four-of-a-kind, and/or three-of-a-kind; determines the number of
 * pairs; stores the results into the external variables straight, flush,
