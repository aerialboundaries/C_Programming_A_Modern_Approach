...
// if new_nord->value is smaller than 1st node, loop does not work
// if new_nord->value is the largest, cur will be null.
...
struct node *insert_into_ordered_list(struct node *list, struct node *new_node)
{
    ...
    // if new_nord->value is the smallest
...
リストが空の場合(list == NULL)
    : cur->value
    を参照した瞬間にセグメンテーションフォールト（強制終了）が発生します。

    リストの末尾まで進む場合
    : new_node->value がリスト内のどの値よりも大きい場合、cur が NULL
      になります。その状態で
      while (cur->value...) を判定しようとするとエラーになります。

      先頭への挿入 : 先頭に挿入する場合、関数の戻り値は
                     new_node （新しい先頭）である必要がありますが、今のコードは
                     return list;
のままで、古い先頭を返してしまいます。

正しい修正方法
これらのケースをすべてカバーするには、while 文の条件に cur
    != NULL を追加し、先頭に挿入する場合の分岐を正しく書く必要があります。
...
安全なループ
    : cur
      != NULL
          を先にチェックすることで、リストの末尾を通り過ぎてエラーになるのを防いでいます。

              先頭挿入への対応
    : prev
      == NULL かどうかで、リストの「頭」がすげ変わったかどうかを判断しています。

          空リストへの対応
    : list が NULL で渡されても、while をスキップして if (prev
          == NULL) に入り、正しく new_node を返します。
    ...
    : 末尾への挿入
          ...
          Neovimでこのコードを書いて、いろいろな値を挿入するテストをしてみてください。while
              文の条件の順番（cur
      != NULL を左側に書くこと）も、ショートサーキット評価といって非常に重要なポイントですよ。
